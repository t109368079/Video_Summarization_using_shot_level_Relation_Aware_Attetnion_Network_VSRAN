...
def get_performance(true_positive, predicted_positive, positive):
    ...
    f1_score = 0 if precision == 0 and recall == 0 else 2 * precision * recall / (precision + recall+1e-10)
...
def mean_average_precision(pred_score, frame_summaries, segment, top=None):
    ...
    for user_summary in frame_summaries:
        ...
        score = mAP(top_summary, top_score) if not np.isnan(top_score).any() else 0
        score = 0 if np.isnan(score) else score
        map_score += score
    map_score /= n_user
    return map_score

def kendall_spearman(pred_shot, user_scores, segment=None):
    ...
    if segment is not None:
        ...
        n_frame = user_scores.shape[1]
        pred_frame = pred_shot
